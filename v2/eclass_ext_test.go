...
func TestEClassSuperTypes(t *testing.T) {
	...
	assert.Equal(t, []EClass{eSuperClass}, eClass.GetESuperTypes().ToArray())
	assert.Equal(t, []EClass{eSuperSuperClass, eSuperClass}, eClass.GetEAllSuperTypes().ToArray())
	...
	eClass.GetESuperTypes().AddAll(NewImmutableEList([]EClass{eSuperClass, eSuperClass2}))
	assert.True(t, containsSubClass(eSuperClass, eClass))
	...
	eClass.GetESuperTypes().RemoveAll(NewImmutableEList([]EClass{eSuperClass, eSuperClass2}))
	assert.False(t, containsSubClass(eSuperClass, eClass))
	assert.True(t, containsSubClass(eSuperClass3, eClass))
}
...
func TestEClassFeaturesGetters(t *testing.T) {
	...
	eFeatures := eClass.GetEStructuralFeatures()
	eFeatures.AddAll(NewImmutableEList([]EStructuralFeature{eAttribute1, eReference1, eAttribute2, eReference2}))
...
func TestEClassFeaturesGettersWithSuperType(t *testing.T) {
	...
	eAttribute2 := newEAttributeExt()
	eReference2 := newEReferenceExt()
	eClass.GetEStructuralFeatures().AddAll(NewImmutableEList([]EStructuralFeature{eAttribute1, eReference1}))
	eSuperClass.GetEStructuralFeatures().AddAll(NewImmutableEList([]EStructuralFeature{eAttribute2, eReference2}))

	// collections
	assert.Equal(t, []EStructuralFeature{eAttribute2, eReference2}, eSuperClass.GetEAllStructuralFeatures().ToArray())
	assert.Equal(t, []EAttribute{eAttribute2}, eSuperClass.GetEAllAttributes().ToArray())
	assert.Equal(t, []EReference{eReference2}, eSuperClass.GetEAllReferences().ToArray())
	assert.Equal(t, []EAttribute{eAttribute2}, eSuperClass.GetEAttributes().ToArray())
	assert.Equal(t, []EReference{eReference2}, eSuperClass.GetEReferences().ToArray())

	assert.Equal(t, []EStructuralFeature{eAttribute2, eReference2, eAttribute1, eReference1}, eClass.GetEAllStructuralFeatures().ToArray())
	assert.Equal(t, []EAttribute{eAttribute2, eAttribute1}, eClass.GetEAllAttributes().ToArray())
	assert.Equal(t, []EReference{eReference2, eReference1}, eClass.GetEAllReferences().ToArray())
	assert.Equal(t, []EAttribute{eAttribute1}, eClass.GetEAttributes().ToArray())
	assert.Equal(t, []EReference{eReference1}, eClass.GetEReferences().ToArray())

	// now remove super type
	eClass.GetESuperTypes().Remove(eSuperClass)

	assert.Equal(t, []EStructuralFeature{eAttribute1, eReference1}, eClass.GetEAllStructuralFeatures().ToArray())
	assert.Equal(t, []EAttribute{eAttribute1}, eClass.GetEAllAttributes().ToArray())
	assert.Equal(t, []EReference{eReference1}, eClass.GetEAllReferences().ToArray())
	assert.Equal(t, []EAttribute{eAttribute1}, eClass.GetEAttributes().ToArray())
	assert.Equal(t, []EReference{eReference1}, eClass.GetEReferences().ToArray())
...
func TestEClassFeaturesGetFromName(t *testing.T) {
	...
	eClass.GetEStructuralFeatures().AddAll(NewImmutableEList([]EStructuralFeature{eAttribute1, eAttribute2}))
...
func TestEClassOperationsGetters(t *testing.T) {
	...
	eOperations.AddAll(NewImmutableEList([]EOperation{eOperation1, eOperation2}))
	...
	// collections
	assert.Equal(t, []EOperation{eOperation1, eOperation2}, eClass.GetEAllOperations().ToArray())
	assert.Equal(t, []EOperation{eOperation1, eOperation2}, eClass.GetEOperations().ToArray())

	// insert another one
	eOperation3 := newEOperationExt()
	eOperations.Insert(0, eOperation3)
	assert.Equal(t, []EOperation{eOperation3, eOperation1, eOperation2}, eClass.GetEAllOperations().ToArray())
	assert.Equal(t, []EOperation{eOperation3, eOperation1, eOperation2}, eClass.GetEOperations().ToArray())
...
func TestEClassOperationsGettersWithSuperType(t *testing.T) {
	...
	// collections
	assert.Equal(t, []EOperation{eOperation2}, eSuperClass.GetEAllOperations().ToArray())
	assert.Equal(t, []EOperation{eOperation2}, eSuperClass.GetEOperations().ToArray())

	assert.Equal(t, []EOperation{eOperation2, eOperation1}, eClass.GetEAllOperations().ToArray())
	assert.Equal(t, []EOperation{eOperation1}, eClass.GetEOperations().ToArray())

	// now remove super type
	eClass.GetESuperTypes().Remove(eSuperClass)

	assert.Equal(t, []EOperation{eOperation1}, eClass.GetEAllOperations().ToArray())
	assert.Equal(t, []EOperation{eOperation1}, eClass.GetEOperations().ToArray())
}
...
func TestEClassAllContainments(t *testing.T) {
	...
	assert.Equal(t, []EReference{eReference2, eReference1}, eClass.GetEAllContainments().ToArray())
...
func TestEClassContainments(t *testing.T) {
	...
	eClass.GetEStructuralFeatures().AddAll(NewImmutableEList([]EStructuralFeature{eReference0, eReference1, eReference2}))

	assert.Equal(t, []EStructuralFeature{eReference1}, eClass.GetEContainmentFeatures().ToArray())
	assert.Equal(t, []EStructuralFeature{eReference0, eReference2}, eClass.GetECrossReferenceFeatures().ToArray())
}
