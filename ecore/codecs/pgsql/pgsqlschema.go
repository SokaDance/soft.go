...
func (t *sqlTable) createQuery() string {
	...
	tableQuery.WriteString(pgsqlEscapeIdentifier(t.name))
	...
	for i, c := range t.columns {
		...
		tableQuery.WriteString(pgsqlEscapeIdentifier(c.columnName))
		tableQuery.WriteString(" ")
	...
	for _, c := range t.columns {
		if c.reference != nil {
			...
			tableQuery.WriteString(pgsqlEscapeIdentifier(c.reference.name))
			tableQuery.WriteString("(")
			tableQuery.WriteString(pgsqlEscapeIdentifier(c.reference.key.columnName))
			tableQuery.WriteString(")")
		}
	}
	tableQuery.WriteString(");")
...
func (t *sqlTable) insertQuery() string {
	...
	if t.key != nil {
		tableQuery.WriteString(pgsqlEscapeIdentifier(t.key.columnName))
	} else {
...
func (t *sqlTable) keyName() string {
	return pgsqlEscapeIdentifier(t.key.columnName)
}
...
func (t *sqlTable) selectQuery(columns []string, selection string, orderBy string) string {
	...
	if len(columns) == 0 {
		...
		for i, column := range columns {
			...
			selectQuery.WriteString(pgsqlEscapeIdentifier(column))
		}
	}
	selectQuery.WriteString(" FROM ")
	selectQuery.WriteString(pgsqlEscapeIdentifier(t.name))
	if len(selection) > 0 {
	...
	if classSchema == nil {
		...
		for itFeature := eFeatures.Iterator(); itFeature.HasNext(); {
			...
			featureSchema := &sqlFeatureSchema{
				feature:     eFeature,
				featureKind: getPGSQLCodecFeatureKind(eFeature),
			}
...
var pgsqlKeyWordsAsSet = func() (result map[string]struct{}) {
	result = map[string]struct{}{}
	for _, keyword := range pgsqlKeywWords {
		result[keyword] = struct{}{}
	}
	return
}()

func isPGSQLKeyWord(k string) bool {
	if _, isKeyWord := pgsqlKeyWordsAsSet[k]; isKeyWord {
		return true
	}
	if _, isKeyWord := pgsqlKeyWordsAsSet[strings.ToUpper(k)]; isKeyWord {
		return true
	}
	return false
}

func pgsqlEscapeIdentifier(id string) string {
	if id[0] == '.' || isPGSQLKeyWord(id) {
		return "\"" + id + "\""
	}
	return id
}
